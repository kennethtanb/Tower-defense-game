...
//    usedSpace = 5;
    ...
    std::string space = "Space: " + std::to_string(usedSpace) + " / " + std::to_string(totalSpace);
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER) {
        ...
        totalSpace = 5;
        std::string space = "Space: " + std::to_string(usedSpace) + " / " + std::to_string(totalSpace);
        UISpaceUsage->Text = space;
        
        for(int i=0;i<totalArmy-1;i++){
            armyAmount[i] = 0;
            space = "0";
            UIArmyAmount[i]->Text = space;
        }
