...
void PlayScene::Initialize() {
    ...
    AddNewObject(MinesGroup = new Group());
...
void PlayScene::Update(float deltaTime) {
    timertime-=deltaTime*SpeedMult;
    
    UISpaceUsage->Text = "Time Remaining: " + std::to_string(int(timertime));
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if(keyCode == ALLEGRO_KEY_UP || keyCode == ALLEGRO_KEY_DOWN || keyCode == ALLEGRO_KEY_LEFT || keyCode == ALLEGRO_KEY_RIGHT || keyCode == ALLEGRO_KEY_ENTER){
        ...
        if(keyStrokes.size()==code.size()){
            for(int i=0;i<code.size();i++){
                if(code[i]!=keyStrokes.front()) right_code = false;
                keyStrokes.pop_front();
            }
            if(right_code) EffectGroup->AddNewObject(new Plane());
        }
    }
...
void PlayScene::ReadMap() {
    ...
    // Read map file.
    for(int i=0;i<4;i++){
        brokenWall[i].erase(brokenWall[i].begin(),brokenWall[i].end());
    }
    char c;
    ...
    while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_ICE_CANNON); break;
        case '4': mapData.push_back(TILE_MINES); break;
        case '\n':
        case '\r':
        ...
        for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_ICE_CANNON:
                    DefenseGroup->AddNewObject(new IceCannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_MINES:
                    MinesGroup->AddNewObject(new MinesDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_FLOOR:
                    if (j <= MapWidth-2 && j >= 2) {
...
void PlayScene::ConstructUI() {
    // Background
    timertime = 60;
    space = "Time\nRemaining: " + std::to_string(int(timertime));
    UIGroup->AddNewObject(new Engine::Image("play/sand.png", 0, 64*MapHeight, 1536, 128));
    UIGroup->AddNewObject(UISpaceUsage = new Engine::Label(space, "pirulen.ttf", 30, 1200, 64*MapHeight+20, 0, 0, 0, 255, 0.5, 0.5));
